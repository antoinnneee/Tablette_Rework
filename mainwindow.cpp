    ...
    ui(new Ui::MainWindow)
{
    ...
    ScanTime2->setSingleShot(0);
    ScanTime3->setSingleShot(0);

    BACKGROUNDSOUND = new QMediaPlayer;
    BACKGROUNDSOUND->setMedia(QUrl::fromLocalFile(GENERALPATH + "son/void.mp3"));
    QTimer::singleShot(500, this, SLOT(TimeToInit()));
    connect(this, SIGNAL(RfcomResult(QString)), this, SLOT(toldmewhatIshoulddo(QString)), Qt::QueuedConnection);
    connect(&client, SIGNAL(newMessage(QString,QString)),this, SLOT(appendMessage(QString,QString)), Qt::QueuedConnection);
//    connect(ScanTime2, SIGNAL(timeout()), this, SLOT(PingTest()));
    connect(ScanTime3,SIGNAL(timeout()),this, SLOT(sendping()), Qt::QueuedConnection);
//    connect(ScanTime, SIGNAL(timeout()), this, SLOT(PingTest()));
    connect(BACKGROUNDSOUND, SIGNAL(mediaStatusChanged(QMediaPlayer::MediaStatus)),this ,SLOT(BGamb(QMediaPlayer::MediaStatus)),Qt::QueuedConnection);
    connect(timer2, SIGNAL(timeout()), this, SLOT(itstime()), Qt::QueuedConnection);
...
{
    nbrping = 0;
    disconnect(ScanTime3,SIGNAL(timeout()),this, SLOT(sendping()));
    disconnect(BACKGROUNDSOUND, SIGNAL(mediaStatusChanged(QMediaPlayer::MediaStatus)),this ,SLOT(BGamb(QMediaPlayer::MediaStatus)));
    disconnect(timer2, SIGNAL(timeout()), this, SLOT(itstime()));
    qDebug() << " try rfcomm";
    //    if (rfcomm->available())
    rfcomm->disconnect();
